...
- This is an L1 report-only loop: do not run `pnpm install`, do not edit
  `package.json` or `pnpm-lock.yaml`. Report findings to
  `dependency-sweeper-state.md` only.
- Use `pnpm audit`, not `npm audit` — this repo uses pnpm.
