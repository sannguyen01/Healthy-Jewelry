...
**No auto-merge at L1** — `gate.yaml`'s `autoMergeAllowlist` is empty on
purpose; the fix phase is disabled entirely at this autonomy level.
...
1. Pause the scheduled task: `Disable-ScheduledTask -TaskName 'DailyTriage-Healthy-Jewelry' -TaskPath '\LoopEngineering\'`
