...
| [029](029-a-governance-clock-is-not-a-merge-gate.md) | A governance clock is not a merge gate | An acceptance-age assertion inside the merge gate turned every pull request red over a calendar date nobody had pushed; spans `control-registry.test.ts`, `probe-accepted-gap.mjs`, `control-audit.yml` and `docs/controls.json` |
| [030](030-an-equivalence-relation-is-the-control.md) | An equivalence relation is the control | Issue #24 took 100 identical comments across ~124 runs and escalated zero times, because the decision lived in a YAML string nothing could import; spans `production-smoke.yml`, `scripts/lib/escalation.mjs`, its test, and the inline-script budget |
...
| [035](035-a-control-outlives-its-subject.md) | A control outlives its subject | Deleting a subject does not delete the controls written against it — it silently changes what they mean; eleven registries refused one decommission commit, and the worst of them went vacuously green; spans `cache-tag-contract`, `failure-mode-registry`, `metadata-data-source`, `typography-weights`, `price-absence-contract`, `docs/failure-modes.md` and the sentinel registry |
| [036](036-a-prohibition-in-prose-is-not-a-boundary.md) | A prohibition in prose is not a boundary | Every prohibition this repository wrote as prose was violated at least once and nothing went red; the decommission's output is an *absence*, which is the one thing nobody notices returning. Spans `COMMERCE-ELIMINATION-CONTRACT.md`, `docs/commerce-dependency-register.md`, `scripts/lib/commerce-contract.mjs`, the route-inventory reconciliation, `docs/controls.json` and the `commerce-boundary` sentinel |
